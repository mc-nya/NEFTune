...
if __name__ == "__main__":
    ...
    parser.add_argument("--file_path",
                        default="datasets/self-instruct-val(processed).jsonl",
                        type=str)
    parser.add_argument(
        "--save_file_name",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        "This reduce the number of generation examples to 4, so that we can debug faster.",
    )
    ...
    model = load_fsdp_ckpt_with_accelerate(
        ...
        if "llama" in args.model else "OPTDecoderLayer",
    )
    ...
    if ("dolly" in args.file_path or "vicuna" in args.file_path
        ...
        raw_data = datasets.load_dataset("tatsu-lab/alpaca_eval",
                                         "alpaca_eval")["eval"]
        # set generator field to model name
        raw_data = raw_data.map(
            lambda x:
            {"generator": args.model_name if args.model_name else args.model})
