...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file_path", default="datasets/self-instruct-val(processed).jsonl", type=str
    )
    parser.add_argument(
        "--save_file_name",
        default="outputs/answers/self-instruct_llama7B.jsonl",
        type=str,
    )
    ...
    model = load_fsdp_ckpt_with_accelerate(
        args.model,
        model_config,
        hf_dummy_path=args.model_config_path,
        wrapped_class="LlamaDecoderLayer"
        if "llama" in args.model
        else "OPTDecoderLayer",
    )
    ...
    if (
        "dolly" in args.file_path
        or "vicuna" in args.file_path
        or "user_oriented_instructions" in args.file_path
    ):
        ...
        raw_data = raw_data.map(
            lambda x: {"generator": args.model_name if args.model_name else args.model}
        )
    ...
    dataset_w_responses = raw_data.map(
        generate, batched=True, batch_size=args.batch_size
    )
    if "alpaca_eval" in args.file_path:
        # stringify the metadata, so that it becomes hashable
        dataset_w_responses = dataset_w_responses.map(
            lambda x: {"metadata": json.dumps(x["metadata"])}
        )
        dataset_w_responses.to_json(
            args.save_file_name, orient="records", lines=False, indent=True, index=False
        )
