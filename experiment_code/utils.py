...
def get_gpu_memory_usage(rank):
    return {
        'total':
        round(
            torch.cuda.get_device_properties(rank).total_memory / (1024**3),
            2),
        'max':
        round(torch.cuda.max_memory_allocated(rank) / (1024**3), 2),
        'reserved':
        round(torch.cuda.memory_reserved(rank) / (1024**3), 2),
        'allocated':
        round(torch.cuda.memory_allocated(rank) / (1024**3), 2)
    }
...
def load_fsdp_ckpt_with_accelerate(fsdp_path, model_config, hf_dummy_path,
                                   wrapped_class):
    ...
    with init_empty_weights():
        model_empty = transformers.AutoModelForCausalLM.from_config(
            model_config)
    ...
    model = load_checkpoint_and_dispatch(
        ...
        hf_dummy_path,
        device_map="auto",
        no_split_module_classes=[wrapped_class, torch.nn.Embedding])
    # this is a hack
    ...
    current_vocab_size_based_on_weight = model.get_output_embeddings(
    ).weight.shape[0]
    ...
    model = load_state_dict_fsdp(model,
                                 fsdp_path,
...
def save_opt_or_scheduler_fsdp(model,
                               opt,
                               save_path,
                               rank,
                               offload_to_cpu=True):
    with FSDP.state_dict_type(
            model, StateDictType.SHARDED_STATE_DICT,
            ShardedStateDictConfig(offload_to_cpu=offload_to_cpu)):
        torch.save(opt.state_dict(), os.path.join(save_path,
                                                  f"shard{rank}.pt"))


def load_opt_or_scheduler_fsdp(model,
                               opt,
                               save_path,
                               rank,
                               offload_to_cpu=True):
...
def save_model_opt_scheduler_states_fsdp(model,
                                         opt,
                                         scheduler,
                                         step_count,
                                         checkpoint_path,
    ...
    if rank == 0:
        if not keep_old_checkpoints:
            remove_all_folders_except_the_latest_step_count(
                checkpoint_path, step_count)


def load_model_opt_scheduler_states_fsdp(model, opt, scheduler,
                                         checkpoint_path):
    last_checkpoint_path, start_step_count = get_last_checkpoint_path_and_last_step_count(
        checkpoint_path)
    # for component, name in [[model, "model"], [opt, "opt"], [scheduler, "scheduler"]]:
...
def remove_all_folders_except_the_latest_step_count(checkpoint_path,
                                                    cur_step_count):
...
def get_last_checkpoint_path_and_last_step_count(checkpoint_path):
    ...
    if os.path.exists(checkpoint_path):
        ...
        last_checkpoint_path = os.path.join(checkpoint_path,
                                            str(last_step_count))
...
def add_padding_token(tokenizer):
    ...
    print("Special tokens before adding padding token: ",
          tokenizer.special_tokens_map)
    if not tokenizer.pad_token:
        tokenizer.add_special_tokens({"pad_token": "[PAD]"})
    print("Special tokens after adding padding token: ",
          tokenizer.special_tokens_map)
    return tokenizer
...
def make_nonpersistent_buffer_persistent(model):
    ...
    for name, module in model.named_modules():
        if hasattr(module, "_non_persistent_buffers_set") and len(
                module._non_persistent_buffers_set) > 0:
            print(
                f"moving non-persistent buffers to persistent buffers for module {name}"
            )
